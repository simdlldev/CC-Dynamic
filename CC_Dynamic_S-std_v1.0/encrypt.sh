...
characters='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789,.-;_?/()=+[]{}|<>:' # 81
encript=""
key=""
debug='0'
shift=42
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
		...
		-h | --help )
			...
			echo -e "Avadable options:\n-t --text 'TEXT'\tProvide the text to encrypt (optional)\n-k --key 'KEY'\t\tProvide a key for the encrypt (optional)"
...
for (( j=0; j<${#text}; j++ )); do
	char1="${text:$j:1}"
	if [[ ! $characters =~ $char1 && $char1 != '['  && $char1 != ']' && $char1 != '('  && $char1 != '{' ]] ; then
		...
		text=$(echo "$text" | sed "s/$char1/$to_sub/g" )
	fi
